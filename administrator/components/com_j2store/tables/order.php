	...
	*/
	...
	public function getOrderDiscountTotals() {
		...
		if ($session->has ( 'coupon', 'j2store' )) {
			$code = $session->get ( 'coupon', '', 'j2store' );
		...
		if ($session->has ( 'voucher', 'j2store' )) {
			$code = $session->get ( 'voucher', '', 'j2store' );
	...
	public function get_discounted_price(&$item, $price, $add_totals = false) {
		...
		if($session->has( 'coupon', 'j2store' )) {
			$coupon_model = F0FModel::getTmpInstance ( 'Coupons', 'J2StoreModel' );
			if ($coupon_model->init () && $coupon_model->is_valid ($this) && $coupon_model->is_valid_for_product ( $item )) {
				...
				if ($add_totals) {
					...
					$this->increase_coupon_discount_amount ( $session->get ( 'coupon', '', 'j2store' ), $total_discount, $total_discount_tax );
		...
		if($session->has( 'voucher', 'j2store' )) {
			...
			if ($voucher_model->init () && $voucher_model->is_valid ()) {
				...
				if ($add_totals) {
					...
					$this->discount_cart += $total_discount;
					$this->discount_cart_tax += $total_discount_tax;
					$this->increase_coupon_discount_amount ( $session->get ( 'voucher', '', 'j2store' ), $total_discount, $total_discount_tax );
					// $this->increase_coupon_applied_count( $code, $values['quantity'] );
				}
			}
		}
	...
	public function get_payment_method() {
		$payment_method = $this->orderpayment_type;
	...
	function get_formatted_subtotal($including_tax = false, $items=array()) {
		...
		if($including_tax) {
			...
			return $this->order_subtotal_ex_tax;
		}
	}
	...
	public function get_formatted_order_totals() {
		...
		if(isset($taxes) && count($taxes) ) {			
			...
			foreach($taxes as $tax) {
				...
				$total_rows['tax_'.F0FInflector::underscore($tax->ordertax_title)] = array(
						'label' => $label,
						'value'	=> $value
				);
	...
	protected function add_line_item( $item_name, $quantity = 1, $amount = 0, $tax=0, $item_number = '', $options=array()) {
		...
		$line_item = array();
		$line_item['name'] = html_entity_decode( trim( $item_name, 127 ), ENT_NOQUOTES, 'UTF-8' );
